...
void computeNormalMapFromHeightMap(FIBITMAP *heights, FIBITMAP *normals, double zScale) {
    ...
    for (int xPrev = w-2, x = w-1, xNext = 0; xNext < w; xPrev=x, x=xNext++) {
        ...
        for (int yPrev = h-2, y = h-1, yNext = 0; yNext < h; yPrev=y, y=yNext++) {
            ...
            //vec3 normal(0);
			vec3 prev = vec3(x, yPrev, pix(heights, x, yPrev));
			vec3 next = vec3(xNext, y, pix(heights, xNext, y));
			vec3 cur = vec3(x, y, pix(heights, x, y));
			vec3 cross1 = prev - cur;
			vec3 cross2 = next - cur;
	
			vec3 normal = cross1 ^ cross2;
			normal[2] = normal[2] * zScale;
			normal.normalize();

			normal[0] = (normal[0] * 127) + 127;
			normal[1] = (normal[1] * 127) + 127;
			normal[2] = (normal[2] * 127) + 127;
